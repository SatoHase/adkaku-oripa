...
export const COLUMNS = [
  "id","site_id","category_id","name","new_user_only","bonus_type","url","price","guarantee_text","guarantee_value",
...
// 手動で status を rejected にした行を skipped に記録して oripa から消す（メールの「不可」と同じ扱い）。
// 先に skipped へ追記してから削除するので、途中で失敗しても id が失われない（重複追記は id で防ぐ）。
export const REJECTED_STATUSES = new Set(["rejected"]);
export async function sweepRejected(sheet, skippedSheet) {
  const rows = await sheet.getRows();
  const rejected = rows.filter((r) => REJECTED_STATUSES.has(String(r.get("status") ?? "").trim().toLowerCase()));
  if (!rejected.length) return 0;
  const known = new Set((await skippedSheet.getRows()).map((r) => r.get("id")));
  const add = rejected
    .filter((r) => !known.has(r.get("id")))
    .map((r) => Object.fromEntries(SKIPPED_COLUMNS.map((c) => [c, c === "first_seen_at" ? new Date().toISOString() : (r.get(c) ?? "")])));
  if (add.length) await skippedSheet.addRows(add);
  await deleteRowsPaced(rejected);
  console.log(`[sheet] rejected -> skipped: ${rejected.map((r) => r.get("id")).join(", ")}`);
